...
import { ACTION_LABELS, type ActionDraft, type ClearActionLabel } from "./actionRules";
...
export function ActionEditor({ draft, onChange, currentFrame, frameReady, saving, editing, error, onSave, onCancel }: {
  ...
  const chooseLabel = (label: ActionDraft["label"]) => update({
    label,
    crossing_frame: ["hand_in", "hand_out"].includes(label) ? latestDraft.current.crossing_frame : null,
  ...
  useEffect(() => {
    const key = (event: KeyboardEvent) => {
      ...
      if (frameReady && event.key.toLowerCase() === "c" && ["hand_in", "hand_out"].includes(latestDraft.current.label)) update({ crossing_frame: currentFrame });
      if (event.key === "Escape") onCancel();
      if (event.ctrlKey && event.key.toLowerCase() === "s") { event.preventDefault(); onSave(latestDraft.current); }
    };
    window.addEventListener("keydown", key);
    return () => window.removeEventListener("keydown", key);
  });
  return <form className="action-editor" onSubmit={(event) => { event.preventDefault(); onSave(latestDraft.current); }}>
    ...
    <div className="editor-actions"><button className="primary" type="submit" disabled={saving}>{saving ? "Đang lưu…" : editing ? "Lưu thay đổi" : "Lưu nhãn"}</button><span><kbd>Ctrl</kbd>+<kbd>S</kbd></span></div>
  </form>;
}
